...
**影像完全相同**：本批唯一符合的是 **A0131 / YT13**（192 張影像完全相同，同一次掃描存成兩份）→ YT13 排除。

**DICOM 檔頭**（FreeSurfer 端逐一讀出）：
- A0132：出生日期、性別、體重與 A0131 相同，相隔 23 天；影像不相同 → 同一位男童的第二次掃描，排除
- D015 / D037：出生日期、體重相同，相隔 3 個月跨生日、年齡 37 → 38 一致 → 同一人（性別欄互相矛盾）
- D038 / DGM002：出生日期、性別、年齡相同，相隔 9 個月 → 同一人
- T029 / T028、D024 / D023、VNT028 / VNT027：同一天相隔約 1 小時、所有欄位完全相同 →
  較可能是技師沿用上一位的登錄，這幾筆的人口學欄位不可用

⚠️ 以前這一節還用標籤 Dice 佐證（例如「影像重疊 0.857」「複製登錄的兩位 Dice 0.63 → 不同人」），
那些數字依規定已刪除，不再作為依據。

### 15.3 DGM 與 VNT

- **DGM**：D015 / D037、D038 / DGM002 依檔頭判為同一人。
  但 mixed_v2 起使用者決定切分不做歸戶，歸戶表 `ASD/DGM_groups.txt` 已刪除。
